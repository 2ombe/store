...
export default function ReportScreen() {
  ...
  useEffect(() => {
    const fetchReport = async () => {
      try {
        ...
        const { data } = await axios.get(`https://final-2wtk.onrender.com/api/report/${reportId}`, {
  ...
  const submitHandler = async (e) => {
    ...
    try {
      ...
      await axios.put(
        `https://final-2wtk.onrender.com/api/report/${reportId}`,
        {
          _id: reportId,
          name: report.name,
  ...
  const handleDelete = async () => {
    if (window.confirm("Are you sure you want to delete this report?")) {
      try {
        await axios.delete(`https://final-2wtk.onrender.com/api/report/delete/${reportId}`, {
          headers: { Authorization: `Bearer ${userInfo.token}` },
        });
  ...
  const handleUpdate = async () => {
    try {
      const response = await axios.put(
        `https://final-2wtk.onrender.com/api/report/given/${reportId}`,
        {
          _id: reportId,
          igice,
          name: report.name,
